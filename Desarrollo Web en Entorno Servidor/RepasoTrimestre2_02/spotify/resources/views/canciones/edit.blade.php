<x-app-layout>
    ...
    <form action="{{ route('canciones.update', $cancion) }}" class="max-w-sm mx-auto" method="POST">
        ...
        <div class="mb-5">
            <label for="duracion" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Duracion en
                segundos</label>
        ...
        <div class="mb-5">
            ...
            @foreach ($artistas as $artista)
                <input name="artistas[{{ $artista->id }}]" type="checkbox"
                    {{ $cancion->artistas->contains('id', $artista->id) ? 'checked' : '' }}>
                {{ $artista->nombre }}<br>
            @endforeach
        </div>
        <div class="mb-5">
            ...
            @foreach ($albumes as $album)
                <input name="albumes[{{ $album->id }}]" type="checkbox"
                    {{ $cancion->albumes->contains('id', $album->id) ? 'checked' : '' }}> {{ $album->nombre }}<br>
            @endforeach
        </div>
        <button type="submit"
